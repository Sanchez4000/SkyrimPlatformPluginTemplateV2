...
if (process.env["DEPLOY_PLUGIN"]?.includes("true")) {
  ...
  if (!skyrimFolder)
    if (fs.existsSync(skyrimPathFromEnvironment))
      skyrimFolder = skyrimPathFromEnvironment;
  if (!skyrimFolder) {
    console.error(
      "Please set valid path to your Skyrim folder in skyrim.json (or SKYRIMPATH environment variable)"
    );
    process.exit(1);
  }

  const outputFile = path.join(outputFolder, outputFilename);
  const skyrimPlatformPluginsDirectory = path.resolve(
    skyrimFolder,
    "Data",
    "Platform",
    "PluginsDev"
  ...
  const pluginDestinationPath = path.join(
    skyrimPlatformPluginsDirectory,
    outputFilename
  );
  ...
  plugins.push(
    new WebpackShellPluginNext({ onAfterDone: { scripts: [copyCommand] } })
  );
  ...
  plugins.push(
    new WebpackShellPluginNext({ onBuildEnd: { scripts: [zipCommand] } })
  );
}
